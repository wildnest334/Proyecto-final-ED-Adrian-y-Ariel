...
import java.util.Stack; // Necesario para la versi?n iterativa de QuickSort
 ...
 * para comparar la eficiencia bajo una restricci?n de tiempo.
 */
public class ConcurrentSortingProject {
    ...
    // --- ENUMERACI?N DE ESTRUCTURAS DE DATOS ---
    public enum DataStructure {
        ARRAY, ARRAY_LIST
    }

    // --- ENUMERACI?N DE ALGORITMOS ---
    ...
    public static class DataGenerator {
        ...
        public DataCollection generate100kRestricted() {
            int[] data = new int[100000];
            for (int i = 0; i < data.length; i++) {
                data[i] = random.nextInt(5) + 1; // N?meros entre 1 y 5
            }
...
//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
    ...
    public static class SortTask implements Runnable {
        ...
        // M?tricas que el hilo recolectar?
        ...
        public void run() {
            ...
            // El hilo se ejecuta en bucle hasta que se cumpla la restricci?n de tiempo
            while (System.currentTimeMillis() < startTimeMillis + durationMillis) {
                ...
                try {
                    ...
                    System.err.printf("ERROR en Hilo %s: StackOverflow (Recursi?n profunda). Terminando hilo.\n", sortType.uniqueName);
                    break; 
                } catch (IndexOutOfBoundsException e) { 
                    System.err.printf("ERROR en Hilo %s: Fallo de ?ndice en la partici?n de QuickSort (%s). Terminando hilo.\n", sortType.uniqueName, e.getMessage());
                    break;
                } catch (Exception e) {
                    System.err.printf("ERROR en Hilo %s: Excepci?n inesperada (%s). Terminando hilo.\n", sortType.uniqueName, e.getMessage());
                    break;
                }
        ...
        // --- M?TODO PARA EJECUTAR ALGORITMO EN ARRAY (int[]) ---
        private void executeArraySort(int[] arr, SortAlgorithm algo) {
            switch (algo) {
                ...
                case QUICK_SORT: 
                    if (arr.length > 1) quickSortIterative(arr, 0, arr.length - 1); // Usamos la versi?n iterativa
                    break;
                case COUNTING_SORT:
                    int maxVal = 0;
        ...
        // --- M?TODO PARA EJECUTAR ALGORITMO EN ARRAYLIST (List<Integer>) ---
        private void executeListSort(List<Integer> list, SortAlgorithm algo) {
            switch (algo) {
                ...
                case QUICK_SORT: 
                    if (list.size() > 1) quickSortListIterative(list, 0, list.size() - 1); // Usamos la versi?n iterativa
                    break;
                case COUNTING_SORT:
        ...
        // ------------------------------------------------------------------
        // --- IMPLEMENTACI?N DE ALGORITMOS PARA ARRAYS PRIMITIVOS (int[]) ---
        // ------------------------------------------------------------------
        ...
        private void quickSortIterative(int[] arr, int low, int high) {
            ...
            while (!stack.isEmpty()) {
                ...
                if (low < high) {
                    ...
                    // Priorizamos el sub-array m?s grande para evitar StackOverflow (aunque ya es iterativo, es buena pr?ctica)
        ...
        // Partici?n de Lomuto (utilizada por QuickSortIterative)
        ...
        // --------------------------------------------------------------------
        // --- IMPLEMENTACI?N DE ALGORITMOS PARA ARRAYLISTS (List<Integer>) ---
        // --------------------------------------------------------------------
        ...
        // Partici?n de Lomuto para List (utilizada por QuickSortListIterative)
    ...
    public static void main(String[] args) {
        ...
        while (durationSeconds <= 0) {
            System.out.print("Por favor, ingrese el tiempo total de ejecuci?n (en segundos): ");
            if (scanner.hasNextLong()) {
                durationSeconds = scanner.nextLong();
                if (durationSeconds <= 0) {
                    System.out.println("El tiempo debe ser un n?mero positivo.");
                }
            } else {
        ...
        System.out.println("\nIniciando la ejecuci?n concurrente de las 12 tareas...");
        ...
        for (Thread thread : threads) {
            try {
                ...
                System.err.println("La ejecuci?n principal fue interrumpida.");
            }
        }

        long actualEndTimeMillis = System.currentTimeMillis();
        System.out.println("-------------------------------------------------------------------------");
        System.out.printf("Prueba finalizada! Duraci?n real: %.2f segundos.\n\n", 
        ...
        // 5. Conclusiones y Comparaci?n
        if (!sortedResults.isEmpty()) {
            System.out.println("\n CONCLUSIONES DE EFICIENCIA:");
            SortTask mostEfficient = sortedResults.get(0);
            
            System.out.printf("  - La combinaci?n M?S eficiente (Rank #1) es: %s\n", 
                              mostEfficient.getSortType().uniqueName);
            System.out.printf("    Raz?n: Orden? %d colecciones en promedio de %.3f ms.\n",
                              mostEfficient.getSortedCollectionsCount(), 
                              mostEfficient.getAverageTimePerSort());

            System.out.println("\n AN?LISIS POR ESTRUCTURA DE DATOS:");
            System.out.println("    La tabla superior permite la comparaci?n directa. En general:");
            System.out.println("    - Se espera que las versiones con ARRAY sean m?s r?pidas debido a la eficiencia");
            System.out.println("      del acceso directo a memoria (O(1)) sobre List.get()/List.set().");
            System.out.println("    - El impacto de esta diferencia ser? mayor en los algoritmos O(n?) que realizan");
            System.out.println("      muchas m?s operaciones de lectura/escritura.");
